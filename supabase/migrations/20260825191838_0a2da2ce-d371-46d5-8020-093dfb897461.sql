    ...
    IF t.default_fields IS NOT NULL AND jsonb_typeof(t.default_fields) = 'array' THEN
      ...
      FOR v_field IN SELECT * FROM jsonb_array_elements(t.default_fields) LOOP
        INSERT INTO public.card_fields (
          card_id, proposal_id, heading, content_html, order_index, field_role, origin, heading_enabled
        ) VALUES (
          ...
          'auto',
          COALESCE((v_field->>'heading_enabled')::boolean, t.key = 'b12.methodologies')
        );
        v_pos := v_pos + 1;
      END LOOP;
    END IF;
