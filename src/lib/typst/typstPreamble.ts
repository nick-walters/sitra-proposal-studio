...
#let he-grid(cols, cells) = {
  ...
  set text(hyphenate: false)
  set par(justify: false)
  // `breakable: false`: the effort matrix is a compact band of coloured cells
  // with rounded top and bottom edges — splitting it across pages would cut
  // the band open. Typst pushes the whole grid to the next page instead, and
  // the `sticky: true` caption above it travels with it.
  block(
  width: he-table-width,
  above: 0pt,
  below: 6pt,
