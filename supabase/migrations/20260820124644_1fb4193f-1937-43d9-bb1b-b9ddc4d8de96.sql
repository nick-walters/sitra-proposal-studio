...
AS $$
  SELECT EXISTS (
    ...
    WHERE u.id = auth.uid()
      ...
      AND lower(split_part(u.email, '@', 2)) = 'sitra.fi'
  );
$$;

REVOKE ALL ON FUNCTION public.caller_is_sitra_staff() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.caller_is_sitra_staff() TO authenticated;
...
BEGIN
  -- Platform owners/admins, or any verified @sitra.fi account, may create proposals
