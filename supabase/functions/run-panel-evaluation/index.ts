...
After the LAST major criterion section above (and before "Overall panel assessment"), include a single line: "**Total score:** ${totalUnweighted} / ${stageKey === "full" ? 15 : 10}${impactWeighting !== 1 ? ` (weighted: ${totalWeighted} / ${(stageKey === "full" ? 5 + 5 * impactWeighting + 5 : 5 + 5 * impactWeighting).toFixed(1)})` : ""}". Place it as a standalone paragraph — do not introduce a new heading for it.
