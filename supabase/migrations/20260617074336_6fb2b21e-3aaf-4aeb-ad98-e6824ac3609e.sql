...
-- Tighten proposal creation: only global owners/admins
...
-- Tighten direct INSERT RLS policy on proposals
DROP POLICY IF EXISTS "Authenticated users can create proposals" ON public.proposals;
CREATE POLICY "Only global admins can create proposals"
ON public.proposals
FOR INSERT
TO authenticated
WITH CHECK (auth.uid() = created_by AND public.is_global_admin(auth.uid()));
