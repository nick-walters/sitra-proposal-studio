...
export function useProposalCustomColors(proposalId: string | null | undefined) {
  ...
  const usedQ = useQuery({
    ...
    queryFn: async (): Promise<string[]> => {
      if (!proposalId) return [];
      return fetchProposalUsedColors(proposalId);
    },
    enabled: !!proposalId,
    staleTime: 0,
  ...
  const removeCustomColor = useCallback(
    async (hex: string) => {
      ...
      const latestUsed = new Set(await fetchProposalUsedColors(proposalId));
      qc.setQueryData(['proposal-in-use-colors', proposalId], Array.from(latestUsed));
      if (latestUsed.has(n)) return; // guarded by a fresh DB scan, not stale React Query data
      const current = qc.getQueryData<string[]>(['proposal-custom-colors', proposalId]) ?? customColors;
