...
BEGIN
  ...
  -- ── GUARD: refuse to overwrite anything edited on the card page ────────────
  -- Detection: content comparison against the legacy source of truth.
  -- A target field is a conflict when it already holds content that differs
  -- from the value this migration would write, or when a field that the
  -- migration owns has been deleted on the card page. Empty targets (freshly
  -- seeded default fields) are always safe to fill.
  WITH b12_cards AS (
    SELECT c.id, c.template_key, c.is_source_fed
  ...
  expected AS (
    -- narrative slot 0 of each subsection card
    ...
    UNION ALL
    -- methodology items on the methodologies card
    SELECT bc.id AS card_id,
           mi.order_index + 1 AS order_index,
           CASE WHEN mi.kind = 'case_placeholder' THEN NULL ELSE NULLIF(btrim(mi.heading), '') END AS heading,
           CASE WHEN mi.kind = 'case_placeholder' THEN '' ELSE COALESCE(mi.content_html, '') END AS content_html
  ...
  SELECT array_agg(msg) INTO v_conflicts FROM (
    ...
    SELECT format('card %s, field position %s: added on the cards page', a.card_id, a.order_index)
     ...
     WHERE a.deleted_at IS NULL
       ...
       AND (btrim(COALESCE(a.content_html, '')) <> '' OR a.heading IS NOT NULL)
  ) conflicts;
  ...
  IF v_conflicts IS NOT NULL AND array_length(v_conflicts, 1) > 0 THEN
    RAISE EXCEPTION 'migrate_b12_to_cards aborted: B1.2 cards for proposal % have been edited since migration (% conflict(s)). First conflicts: %',
      p_proposal_id, array_length(v_conflicts, 1),
      array_to_string(v_conflicts[1:5], '; ');
  END IF;
  -- ───────────────────────────────────────────────────────────────────────────
