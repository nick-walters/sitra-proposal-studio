...
ALTER TABLE public.card_figure
  ADD CONSTRAINT card_figure_caption_kind_check CHECK (caption_kind IN ('figure','table','none'));
...
BEGIN
  ...
  IF p_patch ? 'caption_kind' AND COALESCE(NULLIF(p_patch->>'caption_kind',''),'figure') NOT IN ('figure','table','none') THEN
    RAISE EXCEPTION 'A caption is a figure caption, a table caption, or no caption';
  END IF;
