-- 1. Capture scope: fix the renamed table, add the content added since.
...
AS $function$
  SELECT ARRAY[
    ...
    'card_deletions','proposal_section_guidelines'
  ];
$function$;
...
AS $function$
  ...
  FROM (SELECT (jsonb_each_text(public.capture_scope_predicates())).*) AS e(k, v)
...
DECLARE
  ...
  v_delete_order text[] := ARRAY[
    'member_wp_allocations','wp_draft_deliverable_tasks','wp_draft_task_participants',
    ...
    'proposal_risks','wp_drafts','wp_themes','wp_color_palette','work_packages',
    ...
    'figure_references','figures','section_footnotes','section_content',
...
BEGIN
  ...
  FOREACH v_tbl IN ARRAY v_delete_order LOOP
    ...
    IF v_tbl = 'card_fields' THEN
      -- card_field_versions is an append-only ledger with ON DELETE RESTRICT,
      -- so a field that carries history cannot be deleted. Those are handled
      -- by upsert in the insert phase below; only history-free rows go now.
  ...
  FOR i IN REVERSE array_length(v_delete_order, 1) .. 1 LOOP
    ...
    IF v_tbl = 'card_fields' THEN
      -- Upsert so history-bearing survivors are put back to snapshot state.
      ...
      -- Survivors absent from the snapshot were created afterwards and carry
      -- history: they are moved to the recycle bin rather than destroyed.
  ...
  -- proposals row: restore content, preserve id/created_at/created_by
