-- Restore proposal-level edit rights as a route to participant information.
-- 14 Sep migration 20260914083530 replaced "any proposal editor" with
-- "listed participant member or override", which locked out 13 of the 45
-- SUSIE-Q editors and caused the autosave rejections. The lock stays absolute.
...
BEGIN
  ...
  IF public.can_edit_participant_info(auth.uid(), _participant_id) THEN
    RETURN true;
  END IF;
...
-- Same rule for the set-wise helper the screens use to decide editability.
...
BEGIN
  ...
  IF public.is_proposal_admin(_user_id, _proposal_id)
     OR public.can_edit_proposal(_user_id, _proposal_id) THEN
