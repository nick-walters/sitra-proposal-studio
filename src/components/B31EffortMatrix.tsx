...
export function B31EffortMatrix({ wpData, participants, proposalId }: Props) {
  ...
  return (
    <div>
      ...
      <table className={`${tableStyles} border-collapse [&_th]:border-0 [&_td]:border-x-0 [&_td]:border-y [&_td]:border-gray-200 [&_tr]:border-0 [&_tr:last-child_td]:border-b-0`} style={{ width: 'auto', tableLayout: colWidths.length > 0 ? 'fixed' : 'auto' }} ref={tableRef}>
        <thead>
          <tr>
