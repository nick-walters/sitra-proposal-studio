CREATE TABLE public.ls_mirror_settings (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  proposal_id uuid NOT NULL REFERENCES public.proposals(id) ON DELETE CASCADE,
  cost_line text NOT NULL CHECK (cost_line IN ('C.1', 'C.2', 'C.3.consumables', 'C.3.meetings', 'C.3.dissemination', 'C.3.publication', 'C.3.other')),
  is_mirrored boolean NOT NULL DEFAULT false,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE (proposal_id, cost_line)
);

GRANT SELECT, INSERT, UPDATE, DELETE ON public.ls_mirror_settings TO authenticated;
GRANT ALL ON public.ls_mirror_settings TO service_role;

ALTER TABLE public.ls_mirror_settings ENABLE ROW LEVEL SECURITY;

CREATE INDEX idx_ls_mirror_settings_proposal ON public.ls_mirror_settings(proposal_id);

CREATE TRIGGER update_ls_mirror_settings_updated_at
BEFORE UPDATE ON public.ls_mirror_settings
FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

CREATE POLICY "ls_mirror_settings_select" ON public.ls_mirror_settings
FOR SELECT TO authenticated
USING (public.has_any_proposal_role(auth.uid(), proposal_id));

CREATE POLICY "ls_mirror_settings_insert" ON public.ls_mirror_settings
FOR INSERT TO authenticated
WITH CHECK (public.is_proposal_admin(auth.uid(), proposal_id));

CREATE POLICY "ls_mirror_settings_update" ON public.ls_mirror_settings
FOR UPDATE TO authenticated
USING (public.is_proposal_admin(auth.uid(), proposal_id))
WITH CHECK (public.is_proposal_admin(auth.uid(), proposal_id));

CREATE POLICY "ls_mirror_settings_delete" ON public.ls_mirror_settings
...
AS $function$
  SELECT public.capture_scope_predicates_base()
         || jsonb_build_object(
              ...
              'ls_personnel_roles',     'proposal_id = $1',
              'ls_personnel_effort',    'role_id IN (SELECT id FROM ls_personnel_roles WHERE proposal_id = $1)',
              'ls_cost_items',          'proposal_id = $1',
              'ls_depreciation_items',  'proposal_id = $1',
              'ls_participant_budget',  'proposal_id = $1',
              'ls_wp_budget',           'proposal_id = $1',
              'ls_budget_permission_overrides', 'proposal_id = $1',
              'ls_mirror_settings',     'proposal_id = $1'
            );
$function$;
...
AS $function$
  SELECT public.restore_in_scope_tables_base()
         || ARRAY['case_draft_subsections','table_column_headers',
                  'ls_personnel_roles','ls_personnel_effort','ls_cost_items',
                  'ls_depreciation_items','ls_participant_budget','ls_wp_budget',
                  'ls_budget_permission_overrides','ls_mirror_settings'];
$function$;
...
DECLARE
  ...
  v_delete_order text[] := ARRAY[
    -- lump-sum budget (children first); deleted before participants/wp_drafts,
    -- and therefore inserted after them
    'ls_personnel_effort','ls_personnel_roles',
    'ls_cost_items','ls_depreciation_items',
    'ls_wp_budget','ls_participant_budget','ls_budget_permission_overrides',
    'ls_mirror_settings',
    ...
    'case_draft_subsections','case_drafts',
    'methodology_linked_activities','methodology_items','methodology_subsections',
    ...
    'table_captions','table_column_widths','table_column_headers',
...
BEGIN
  ...
  FOR i IN REVERSE array_length(v_delete_order, 1) .. 1 LOOP
    ...
    ELSIF v_tbl IN ('ls_cost_items','ls_depreciation_items') THEN
      -- these tables have STORED GENERATED columns which cannot be inserted into
      ...
      EXECUTE format(
        ...
        v_tbl, v_collist, v_collist, v_tbl
      ) USING v_rows;
      GET DIAGNOSTICS v_ins = ROW_COUNT;
    ELSE
      EXECUTE format(
  ...
  IF v_snap_prop IS NOT NULL THEN
    UPDATE proposals SET
      ...
      lump_sum_budget_active = COALESCE((v_snap_prop ->> 'lump_sum_budget_active')::boolean, lump_sum_budget_active),
      traditional_budget_locked = COALESCE((v_snap_prop ->> 'traditional_budget_locked')::boolean, traditional_budget_locked),
      lump_sum_budget_locked = COALESCE((v_snap_prop ->> 'lump_sum_budget_locked')::boolean, lump_sum_budget_locked),
      ls_indirect_cost_rate = COALESCE(NULLIF(v_snap_prop ->> 'ls_indirect_cost_rate','')::numeric, ls_indirect_cost_rate),
      ls_default_funding_rate = COALESCE(NULLIF(v_snap_prop ->> 'ls_default_funding_rate','')::numeric, ls_default_funding_rate),
      updated_at = now()
