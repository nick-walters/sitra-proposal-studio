...
DECLARE
  ...
  v_payload jsonb; v_next integer; v_order integer;
BEGIN
  ...
  ELSIF d.target_type = 'wp_draft_task' THEN
    ...
    -- Lock the parent work-package row BEFORE reading max(number): two
    -- concurrent restores would otherwise compute the same next number and one
    -- would fail the unique constraint on (wp_draft_id, number).
    PERFORM 1 FROM public.wp_drafts WHERE id = v_wp FOR UPDATE;
    IF NOT FOUND THEN
      RETURN jsonb_build_object('ok', false, 'error', 'parent_missing');
    END IF;

    -- Deletion resequenced the survivors, so the original number is usually
    -- taken. Append to the end; the resequence below tidies the sequence up.
    SELECT COALESCE(MAX(number), 0) + 1, COALESCE(MAX(order_index), 0) + 1
      INTO v_next, v_order
      FROM public.wp_draft_tasks WHERE wp_draft_id = v_wp;
    v_payload := jsonb_set(
                   jsonb_set(d.payload, ARRAY['number'], to_jsonb(v_next), true),
                   ARRAY['order_index'], to_jsonb(v_order), true);
